...
class LoopModel 
{
	 ...
	 *
	 */
	public function fetchAllPDOQuery($sql, $param_array)
	{
		$this->connect();
		$dbStatment = $this->db->prepare($sql);
		if ($dbStatment->execute($param_array)){  //PDO method
	 ...
	 *
	 */
	public function fetchPDOQuery($sql, $param_array)
	{
		$this->connect();
		$dbStatment = $this->db->prepare($sql);
		if ($dbStatment->execute($param_array)){  //PDO method
	...
	*/
	...
	public function getRegisters($_search_array=null, $_search=null, $order="", $attributes = "*")
	{
		$sql = "SELECT $attributes from $this->table where 1  ";
		$param_array = array();
		if ($_search_array) {
			foreach ($_search_array as $key => $search) {
				if (!isset($search['operator'])) $search['operator'] = '=';
				$sql .= " AND  $key {$search['operator']} ? ";
				array_push($param_array, $search['value']);
			}
		}

		return $this->fetchAllPDOQuery($sql,$param_array);
 ...
 *
 */
public function update($id,$array_update,$search_key = 'id')
{
